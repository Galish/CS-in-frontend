...
describe('LRUCache', () => {
	...
	it('#3', () => {
		...
		cache.get(key4) 		// 5, 3, 4
		cache.get(key5) 		// 3, 4, 5
		cache.set(key1, '1')	// 1, 3, 4
		cache.get(key5) 		// 1, 3, 4 - undefined
		cache.get(key3) 		// 1, 4, 3
		cache.set(key2, '2')	// 2, 1, 4

		expect(cache.get(key1)).toBe('1')
		expect(cache.get(key2)).toBe('2')
		expect(cache.get(key3)).toBe(undefined)
		expect(cache.get(key4)).toBe('4')
		expect(cache.get(key5)).toBe(undefined)
	})
